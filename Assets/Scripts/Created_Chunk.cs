...
public class Created_Chunk : Chunk
{
    ...
    private void Initialize_Chunk()
    {
        ...
        Initialize_Vectors();
        Chunks_Controller.Setting_Up_Coordinats(ref Coord_x, ref Coord_z, gameObject.transform); //!!!!!!!Need class Having_Coordinates and overload
    ...
    private void Diamond_Square(int mesh_size, int coord_offset = 0)
    {
        ...
        if (mesh_size == size)
        {
            ...
            Vecs[0].y += (Random.Range(-range,range) + Random.Range(-smaller_range, smaller_range) * multiply) * Not_calculated_vecs[0];
            Not_calculated_vecs[0] = 0;

            Vecs[right_down_corner_index].y += (Random.Range(-range, range) + Random.Range(-smaller_range, smaller_range) * multiply) * Not_calculated_vecs[right_down_corner_index];
            Not_calculated_vecs[right_down_corner_index] = 0;

            Vecs[left_up_corner_index].y += (Random.Range(-range, range) + +Random.Range(-smaller_range, smaller_range) * multiply) * Not_calculated_vecs[left_up_corner_index];
            Not_calculated_vecs[left_up_corner_index] = 0;

            Vecs[right_up_corner_index].y += (Random.Range(-range, range) + Random.Range(-smaller_range, smaller_range) * multiply) * Not_calculated_vecs[right_up_corner_index];
        ...
        Vecs[middle_index].y += ((Vecs[left_down_corner_index].y + 
            ...
            Vecs[right_up_corner_index].y) / 4 + Random.Range(-smaller_range, smaller_range) * multiply) * Not_calculated_vecs[middle_index];
        Not_calculated_vecs[middle_index] = 0;

        Vecs[up_index].y += ((Vecs[left_up_corner_index].y + Vecs[middle_index].y + Vecs[right_up_corner_index].y) / 3 + Random.Range(-smaller_range, smaller_range) * multiply) * Not_calculated_vecs[up_index];
        Not_calculated_vecs[up_index] = 0;

        Vecs[left_index].y += ((Vecs[left_up_corner_index].y + Vecs[middle_index].y + Vecs[left_down_corner_index].y) / 3 + Random.Range(-smaller_range, smaller_range) * multiply) * Not_calculated_vecs[left_index];
        Not_calculated_vecs[left_index] = 0;

        Vecs[right_index].y += ((Vecs[right_up_corner_index].y + Vecs[middle_index].y + Vecs[right_down_corner_index].y) / 3 + Random.Range(-smaller_range, smaller_range) * multiply) * Not_calculated_vecs[right_index];
        Not_calculated_vecs[right_index] = 0;

        Vecs[down_index].y += ((Vecs[left_down_corner_index].y + Vecs[middle_index].y + Vecs[right_down_corner_index].y) / 3 + Random.Range(-smaller_range, smaller_range) * multiply) * Not_calculated_vecs[down_index];
    ...
    private void Initialize_Vectors()
    {
        for(int i = 0; i <= size; i++)
        {
            ...
            {
                Vecs[(i*(size + 1)) + j] = new Vector3 (j * size_normaller , 0, i * size_normaller); //coord for ever dot
            }
        }
    }
}